...
function GalleryCard({ property }) {
  return (
    <Link to={`/propiedad/${property.id}`} className="group block rounded-2xl overflow-hidden relative">
      <div className="aspect-[16/10] overflow-hidden">
        <img src={property.fotos?.[0]} alt={property.titulo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
      </div>
      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <h3 className="font-display font-bold text-white text-sm line-clamp-1">{property.titulo}</h3>
        <p className="text-white/70 text-xs">{property.comuna}</p>
        <span className="font-display font-bold text-lg text-white">{formatPrice(property.precio)}<span className="text-sm font-normal text-white/60">/mes</span></span>
      </div>
    </Link>
  )
}
...
export default function Search({ user }) {
  ...
  const handleShareWhatsApp = () => {
    const text = encodeURIComponent(`Mira estos arriendos en Rentu: ${window.location.href}`)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={shareRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 shadow-sm transition-colors"
              ...
              {showShareMenu && (
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl shadow-black/8 py-1 z-50 animate-fadeIn">
                  <button onClick={handleCopyLink} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  <button onClick={handleShareWhatsApp} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            ...
            <div className="flex bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden" role="group" aria-label="Vista de resultados">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-all ${viewMode === 'gallery' ? 'bg-brand-600 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            ...
            {shareToast && (
              <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 text-sm font-medium rounded-xl shadow-lg animate-fadeIn">
                Link copiado
              </div>
            )}
